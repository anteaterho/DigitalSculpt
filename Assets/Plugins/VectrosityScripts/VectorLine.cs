...
public class VectorLine {
	public static string Version () {
		return "Vectrosity version 4.3.2";
	}
	...
	static void SetCanvas3D (int id) {
		...
		while (m_canvases3D.Count < id+1) {
			var go = new GameObject((m_canvases3D.Count == 0)? "VectorCanvas3D" : "VectorCanvas3D_"+m_canvases3D.Count);
			go.layer = LayerMask.NameToLayer ("UI");
			var canvas = go.AddComponent<Canvas>();
	...
	private void SetIntersectionPoint (int p1, int p2, int p3, int p4) {
		...
		if (d > -0.005f && d < 0.005f) {	// Sometimes nearly parallel lines have errors, so just average the points together
			m_UIVertices[p2].position = (l1b + l2a) * 0.5f;
	...
	private void SetIntersectionPoint3D (int p1, int p2, int p3, int p4) {
		...
		float d = (l2b.y - l2a.y)*(l1b.x - l1a.x) - (l2b.x - l2a.x)*(l1b.y - l1a.y);
		if (d > -0.005f && d < 0.005f) {	// Sometimes nearly parallel lines have errors, so just average the points together
			m_UIVertices[p2].position = cam3D.ScreenToWorldPoint ((l1b + l2a) * 0.5f);
			m_UIVertices[p3].position = m_UIVertices[p2].position;
			return;
		}
		float n = ( (l2b.x - l2a.x)*(l1a.y - l2a.y) - (l2b.y - l2a.y)*(l1a.x - l2a.x) ) / d;
	...
	public void MakeRoundedRect (Vector3 bottomLeft, Vector3 topRight, float cornerRadius, int cornerSegments, int index) {
		...
		int neededCount = m_continuous? cornerSegments * 4 + 5 + index : cornerSegments * 8 + 8 + index;
		...
		int cornerPointCount = m_continuous? cornerSegments + 1 : cornerSegments * 2;
		int originalCount = m_continuous? cornerSegments : cornerSegments * 2;
		if (m_is2D) {
